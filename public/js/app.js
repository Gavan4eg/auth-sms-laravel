    ...
    (() => {
        ...
        let formValidate = {
            ...
            phoneTest(formRequiredItem) {
                return !/^\+38\(\d{3}\)-\d{3}-\d{2}-\d{2}?$/.test(formRequiredItem.value);
            }
        };
        function formSubmit() {
        ...
        function applyPromoCode() {
            ...
            var promoText = promoInput.value.trim().toLowerCase();
            var promoCodeBlock = document.querySelector(".form__price--promocode");
            var formItem = document.querySelector(".form__item--promo");
            var existingError = formItem.querySelector(".form__error");
            if (existingError) existingError.remove();
            if (promoText === "phpua") {
                promoCodeBlock.style.display = "block";
                formItem.classList.remove("_form-error");
            } else {
                promoCodeBlock.style.display = "none";
                formItem.classList.add("_form-error");
                var errorMessage = `<div class="form__error">${promoInput.dataset.error}</div>`;
                formItem.insertAdjacentHTML("beforeend", errorMessage);
            }
        }
        document.addEventListener("DOMContentLoaded", (function() {
            var applyButton = document.querySelector(".form__button--apply");
            if (applyButton) applyButton.addEventListener("click", applyPromoCode);
        }));
        let script_form = document.getElementById("myForm");
        if (script_form) document.querySelectorAll(".number_input").forEach((function(input) {
            input.addEventListener("input", (function(e) {
                let value = this.value;
                if (value.length > 1) this.value = value.charAt(0);
                ...
                } else if (value.length === 0 && prevTabIndex !== 0) {
                    let prevElement = document.querySelector('[tabindex="' + prevTabIndex + '"]');
                    if (prevElement) prevElement.focus();
                }
            }));
            script_form.addEventListener("submit", (function(event) {
                let formData = "";
                document.querySelectorAll(".number_input").forEach((function(input) {
                    formData += input.value;
                }));
                let hiddenInput = document.querySelector(".number_input--hidden");
                if (hiddenInput) hiddenInput.value = formData;
            }));
        }));
        window["FLS"] = false;
        isWebp();
